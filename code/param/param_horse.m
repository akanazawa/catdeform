function param = param_horse(img_set, major_param)
%%%%%%%%%%%%%%%%%%%%
...
if isfield(major_param, 'algorithm')
    ...
    param.algorithm = AlgorithmEnum.CAM_N_FRAME;
    % param.algorithm = AlgorithmEnum.WARM;
    % param.algorithm.vals = 5*logspace(-2, 0, 10);
end
...
%%----------
% horse0 has 2720 vertices and 5436 faces.
% Cat's keep ratio is 0.0743, which we get with targetfacecount ~ 400.
if isfield(major_param, 'tetgen')
    ...
    param.tetgen_targetfacecount = 500;%700;%500
    param.tetgen_maxvol = 10;
end
param.tetgen_maxtetcount = 5000;
param.tetgen_maxretries = 50;
% AJ: needed for certain mesh that has problem with intersecting faces.
param.tetgen_repairmesh = true;
% param.CM_type = 'iso2mesh'; %{'iso2mesh','bb_aligned'}
param.CM_type = 'manual_iso2mesh'; %{'iso2mesh','bb_aligned'}
if strcmp(param.CM_type, 'manual_iso2mesh')
    param.tetgen_simplified_mesh = ['../data/models/myhorse0_v305f606.ply'];
    param.tetgen_targetfacecount = 500;
    param.tetgen_maxvol = 1;
end

param.CM_coordsType = 'mls'; %{'mls','barycentric','mls_euclidean'}
param.CM_mlsSimga = 0.4; % 9 if uncentered.
param.CM_mlsOrder = 1;
...
if param.articulation == ArticulationEnum.LEARN 
    ...
    if param.symmetric_stiffness
        sym_path = '../data/models/symmetric_stiffness_id_myhorse0_v305f606-tetface_500_maxvol_1_maxcount_5000_manual_iso2mesh_mls_sig_0.4.mat';
        t = load(sym_path);
...
%% Model settings
%%----------
param.off_file = ['../data/models/myhorse0.ply'];
...
%% Visualize the annotation in 2D and 3D.
if 0
...
%% Save names.
% $Base = results/<ALG_TYPE>/horse_<tettrgt>_<maxvol>
...
%%----------
param.base_dir = sprintf('../results/%s/horse_%s/', str.algorithm, str.tet);
% Save tet & base CM.
