function [vertex,face, misc] = read_vtk_more(filename, verbose)
...
str = fgets(fid);
misc = struct('info', []);
while ischar(str) & nface ~=0
    ok = regexp(str, '.*SCALARS*');
    if (ok)
        fieldname = sscanf(str,'%*s %s %*s %*s', 1);
        str = fgets(fid); % LOOKUP_TABLE..        
        [A,cnt] = fscanf(fid,'%f\n', nface);
        if cnt == nface
            misc.(fieldname) = A;
        end
